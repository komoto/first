...
function onReady(){	
	console.log("onReady start! chart2");
	xInitD3();
}

function xInitD3(){
	// SVG作成
	svg = d3.select("#result2")
		.append("svg")
		.attr("width", w)
		.attr("height", h);
	//csv読み込み
  d3.csv("data/g2.csv", function(error, data) {
	  csvData=data;
		...
		var isMatch=false;
		 for(var j=0;j<countryList.length;j++){
		  ...
		  if(isMatch==false){  
			...
			$("#chartHolder2 .c"+countryList.length+" .countryName").text(country);
			$("#chartHolder2 .c"+countryList.length+" .ccolor").css("background-color",colorList[countryList.length-1]);
		  }
		...
		isMatch=false;
		 for(var k=0;k<qualificationList.length;k++){
		  ...
		  if(isMatch==false){  
			...
			$("#chartHolder2 .q"+qualificationList.length).text(qualification);
		  }
	  ...
	  // イベント設定
	  $("#chartHolder2 .qBtn").on("click",onQualificationBtnClick);
	  $("#chartHolder2 .cBtn").hover(
		  function () {
	  ...
	  $("#chartHolder2 .cBtn").on("click",onCountryBtnClick);
...
function onCountryBtnClick(){
	...
	if(num==1){
		//全てON
		for(var i=0;i<countryList.length;i++){
			$("#chartHolder2 .countryList .c"+(i+1)).css("opacity",1);
			svg.selectAll(".c"+i+"_line")
			.attr("opacity",1)
			.attr("stroke-width", 1);
		}
		$("#chartHolder2 .countryList .c"+num).css("opacity",1);
			svg.selectAll(".c"+(num-1)+"_line")
			.attr("opacity",1)
			.attr("stroke-width", 1);
			tempAlpha=1;
	...
	var allOn=true; 
	for(var i=0;i<countryList.length;i++){
		var opa=$("#chartHolder2 .countryList .c"+(i+1)).css("opacity");
		if(opa!=1){
			allOn=false;
			break
		}
	...
	if(allOn){
		...
		for(var i=0;i<countryList.length;i++){
			if(i!=0){
			$("#chartHolder2 .countryList .c"+(i+1)).css("opacity",0.3);
			}
			svg.selectAll(".c"+i+"_line")
			.attr("opacity",0.0)
			.attr("stroke-width", 1);
		...
		var cur=$("#chartHolder2 .countryList .c"+num).css("opacity");
		if(cur==1){
			$("#chartHolder2 .countryList .c"+num).css("opacity",0.3);
			svg.selectAll(".c"+(num-1)+"_line")
			.attr("opacity",0.0)
			.attr("stroke-width", 1);
			tempAlpha=0.0;
...
function onQualificationBtnClick(){
	...
	$("#chartHolder2 li").removeClass("selected");
	var num=$(this).attr("class").split(" q")[1];
	
	$("#chartHolder2 li .q"+num).parent().addClass("selected");
...
function xChangeGraph(data,$countryNum){
	...
	svg.selectAll("#chartHolder2 .axis_x").remove();
	...
	svg.selectAll("#chartHolder2 .axis_y").remove();
	...
	svg.selectAll("#chartHolder2 .c"+$countryNum+"_line")
			.transition()
			.duration(1000)
			.attr("d", line(data));
}
